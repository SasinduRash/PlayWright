...
test('Apply Underwriter license with valid data', async ({ page }) => {
  ...
  await page.locator('div').filter({ hasText: /^Underwriter$/ }).nth(1).click();
  ...
  await page.getByRole('textbox', { name: 'Write...' }).fill('Test sttsts tstst stts ststst ststst ststts');
  ...
  await page.getByRole('textbox', { name: 'Enter Details' }).fill('Test');
  ...
  await selectRadio(page, 'the SEC or any other professional or regulatory body?', 0);
  await selectRadio(page, 'the SEC to be licenced as a Market Intermediary and been rejected by the SEC?', 0);
  await selectRadio(page, 'the applicable Rules of the SEC?', 0);
